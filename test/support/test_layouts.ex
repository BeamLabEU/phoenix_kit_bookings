defmodule PhoenixKitBookings.Test.Layouts do
  @moduledoc """
  ...
  # Host layout stand-in for core's `LayoutWrapper.app_layout` (configured
  # via `config :phoenix_kit, layout:` in config/test.exs). Without it the
  # wrapper falls back to core's own root layout, which calls
  # `PhoenixKitWeb.Endpoint.static_path/1` — an endpoint that isn't
  # started in module tests.
  def public(assigns) do
    ~H"""
    <div id="test-public-layout">{@inner_content}</div>
    """
  end
